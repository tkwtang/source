...
def flux_qubit_1D_non_linear_approx_pot(phi_1, params):

    U0_1, gamma, beta, delta_beta, phi_1x, phi_1xdc, x_c, shift = params
    U0_kBT_ratio = U0_1
    U0_1 = 1

    return U0_kBT_ratio * (1/2 * (phi_1 - phi_1x)**2 + beta * np.cos(phi_1) * np.cos(phi_1xdc/2) + shift)
    

def flux_qubit_1D_non_linear_approx_force(phi_1, params):
    
    U0_1, gamma, beta, delta_beta, phi_1x, phi_1xdc, x_c, shift = params

    U_dp1 = (phi_1 - phi_1x) - beta * np.sin(phi_1) * np.cos(phi_1xdc/2)
